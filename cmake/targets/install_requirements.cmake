...
message(STATUS "Determining which reference of the 'docs' repository to switch to...")
...
message(STATUS "Switching to the reference '${SWITCH_POT_REFERENCE}' on the local branch 'current' of the 'docs' repository...")
remove_cmake_message_indent()
message("")
...
message(STATUS "Determining which reference of the 'conan' repository to switch to...")
file(READ "${REFERENCES_JSON_PATH}" REFERENCES_JSON_CNT)
get_reference_of_latest_from_repo_and_current_from_json(
...
if (MODE_OF_UPDATE STREQUAL "COMPARE")
    if (NOT CURRENT_CONAN_REFERENCE STREQUAL LATEST_CONAN_REFERENCE)
        set(SWITCH_CONAN_REFERENCE  "${LATEST_CONAN_REFERENCE}")
    else()
        set(SWITCH_CONAN_REFERENCE  "${CURRENT_CONAN_REFERENCE}")
    endif()
elseif (MODE_OF_UPDATE STREQUAL "ALWAYS")
    set(SWITCH_CONAN_REFERENCE      "${LATEST_CONAN_REFERENCE}")
elseif (MODE_OF_UPDATE STREQUAL "NEVER")
...
message("MODE_OF_UPDATE           = ${MODE_OF_UPDATE}")
message("SWITCH_CONAN_REFERENCE   = ${SWITCH_CONAN_REFERENCE}")
message("")
restore_cmake_message_indent()


message(STATUS "Switching to the reference '${SWITCH_CONAN_REFERENCE}' on the local branch 'current' of the 'conan' directory...")
remove_cmake_message_indent()
message("")
switch_to_git_reference_on_branch(
    IN_LOCAL_PATH   "${PROJ_OUT_REPO_CONAN_DIR}"
    IN_REFERENCE    "${SWITCH_CONAN_REFERENCE}"
    IN_BRANCH       "current")
message("")
restore_cmake_message_indent()
...
foreach(SRC_PATCH_FILE ${SRC_PATCH_FILES})
    ...
    if     (PATCH_FILE_RELATIVE MATCHES "\\.rmfile$")
        string(REGEX REPLACE "\\.rmfile$" "" PATCH_FILE_RELATIVE "${PATCH_FILE_RELATIVE}")
        set(DST_PATCH_FILE "${DST_PATCH_DIR}/${PATCH_FILE_RELATIVE}")
        file(REMOVE "${DST_PATCH_FILE}")
        message("Removed: ${DST_PATCH_FILE}")
    elseif (PATCH_FILE_RELATIVE MATCHES "\\.rmdir$")
        string(REGEX REPLACE "\\.rmdir$" "" PATCH_FILE_RELATIVE "${PATCH_FILE_RELATIVE}")
        set(DST_PATCH_FILE "${DST_PATCH_DIR}/${PATCH_FILE_RELATIVE}")
        file(REMOVE_RECURSE "${DST_PATCH_FILE}")
        message("Removed: ${DST_PATCH_FILE}/")
    else()
        set(DST_PATCH_FILE "${DST_PATCH_DIR}/${PATCH_FILE_RELATIVE}")
        ...
        message("Patched: ${DST_PATCH_FILE}")
    endif()
endforeach()
...
execute_process(
    COMMAND ${Conda_EXECUTABLE} install
            ...
            conda-forge::enchant=${VERSION_OF_ENCHANT}
            --channel conda-forge
            --prefix ${PROJ_CONDA_DIR}
            --yes
    ECHO_OUTPUT_VARIABLE
...
if (VERSION VERSION_LESS_EQUAL    "1.62" AND
    VERSION VERSION_GREATER_EQUAL "1.26")
    set(PIP_INSTALL_EXTRA_PACKAGES  sphinx-sitemap==2.1.0)
endif()
set(REQUIREMENTS_PATH "${PROJ_OUT_REPO_DIR}/requirements.txt")
file(READ "${REQUIREMENTS_PATH}" REQUIREMENTS_CNT)
remove_cmake_message_indent()
message("")
message("PIP_INSTALL_EXTRA_PACKAGES: ${PIP_INSTALL_EXTRA_PACKAGES}")
message("")
